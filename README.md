...
在 WSL2 + `qemu-riscv32` 上，`tests/cases/perf/prime.tc` 的最近一次验证结果为：项目 `-opt`
产物与 GCC `-O2` 产物的退出码均为 `192`，最佳用时分别为 `0.32s` 与 `0.18s`，性能比
`min(1, gcc-O2/ours) = 0.56`。该时间受机器与 QEMU 版本影响，提交前应在目标环境复测。
...
- 后端：RISC-V32 代码生成（D）
- 后端优化：寄存器分配、常量折叠/传播、短路跳转、强度削弱、窥孔、寄存器直连、`addi` 自更新、单参数直传与小栈帧（D）
