...
- **栈帧**：被调用者保存 `ra`/`s0`，`s0` 作帧指针。函数体内 **`sp` 保持不变**——局部变量与
  表达式临时值都用 `s0` 相对固定槽位寻址，因此每个调用点 `sp` 天然 16 字节对齐。临时槽数由
  一趟表达式深度分析预估，帧大小向上取整到 16。
- **变量解析**：后端自带作用域栈完成名字解析（同名遮蔽、块作用域）。常量内联为立即数不占存储；
  全局变量走 `.data` + `la`/`lw`/`sw`；局部/形参走栈槽。形参前 8 个来自 `a0..a7`、其余来自
  调用者栈。
...
- 功能测试：`bash tests/scripts/run_tests.sh [-opt]`。以本机 `gcc` 把 `.tc` 当 C 编译运行得到期望
  退出码，与本编译器产物在 `qemu-riscv32` 上的退出码逐一比对。当前 19 个功能用例在 `-opt` 与
  非 `-opt` 下均全部通过。
