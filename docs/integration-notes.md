...
- **优化（`-opt`）**：常量折叠/传播（含全局常量内联）、乘 2 的幂强度削弱为移位、寄存器变量之间的
  ...
  **叶子函数无栈帧**（局部改放 `a1..a7`）与 `if`/`while` 的**条件落空布局 + 循环旋转**。
  ...
  求值）及**自尾递归消除**（新实参先暂存再回填形参并回跳入口）。不实施需要全局数据流分析的
  通用复制传播、循环外提或全局 CSE。随后补充**局部前向常量传播**（未知分支/循环清除可变常量
  事实）、**相邻纯 CSE**（控制流、调用、作用域切换立即失效）及**块作用域存储复用**。
...
- 性能对比：`bash tests/scripts/perf_compare.sh [用例.tc]`（默认 `tests/cases/perf/prime.tc`）。
  ...
  两者退出码均为 192；最终复测的 3 次取最小结果为本项目 `0.21s`、GCC `-O2` `0.18s`，
  性能得分 `min(1, 基准/实际)` 为 `0.86`，与本轮开始前一致。该用例未命中尾递归热点，
  因而本轮新增规则在该基准上的可量化提升为 0%。另有 100,000 次自尾递归压力用例：返回值为
  80，且生成的递归函数体无 `call` 自身。
