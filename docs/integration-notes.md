...
- **寄存器分配（性能优化）**：局部变量与形参优先分配到被调用者保存寄存器 `s1..s11`（跨函数调用
  ...
  常驻寄存器，显著减少访存。**叶子函数**（不含任何调用）省去 `ra` 的保存与恢复；进一步地，若叶子函数
  局部数 ≤ 7 且表达式嵌套浅，则把全部局部改放调用者保存寄存器 `a1..a7`，**完全省去栈帧与序言/尾声**
  （只用调用者保存寄存器、不触碰 `s0..s11`，天然保持被调用者保存寄存器不变），是热点小函数最大的收益点。
- **变量解析**：后端自带作用域栈完成名字解析（同名遮蔽、块作用域）。常量内联为立即数不占存储；
  全局变量走 `.data` + `la`/`lw`/`sw`；局部/形参走 `s` 寄存器或栈槽。形参前 8 个来自 `a0..a7`、
  其余来自调用者栈。
- **控制流**：`if`/`while` 的条件用 `genCondJump` 生成到目标标签的**单条**分支并让常见路径顺序落下
  （落空布局，省去多余无条件跳转），关系运算融合为 `blt/bge/beq/bne` 并按需取反；`&&`/`||` 按短路语义
  生成；`while` 采用**循环旋转**（条件下沉到底部，稳态每轮仅一条回跳分支）；`break`/`continue` 用循环标签栈。
- **函数调用**：实参从左到右求值后经栈槽装入 `a0..a7`，超过 8 个的溢出到栈（并临时保持 16 对齐）；
  支持递归。`-opt` 对单参数调用直接使用 `a0` 传参，省去无意义的临时栈槽存取。
- **优化（`-opt`）**：常量折叠/传播（含全局常量内联）、乘 2 的幂强度削弱为移位、寄存器变量之间的
  直接二元运算与比较、`x = x +/- 常量` 到 `addi` 的归约，以及删除 `mv x,x`、紧邻标签跳转和紧邻
  冗余加载的窥孔清理。小于等于 2047 字节的栈帧使用短序言/尾声；大帧自动回退到通用路径。除法不做
  移位化简（有符号截断除法与算术右移对负数不等价）。此外还包括四项热路径优化：**小叶子函数内联**
  （不含调用、局部+形参 ≤ 8、体量小的函数在调用点展开，`call/ret` 与参数搬运全部消失）、
  **循环条件常量提升**（循环比较的字面量装入空闲 `s` 寄存器，每轮省一条 `li`）、
  **叶子函数无栈帧**（局部改放 `a1..a7`）与 `if`/`while` 的**条件落空布局 + 循环旋转**。
  本轮补充**安全代数化简/死语句删除**（单位元、零元、相同纯表达式、常量控制流；含调用时仍保留
  求值）及**自尾递归消除**（新实参先集中求值再回填形参并回跳入口：浅表达式经 `t0..t2` 零访存中转、
  恒等实参跳过，复杂情形退回栈临时槽）。随后补充**局部前向常量传播**、**相邻纯 CSE**及
  **块作用域存储复用**。
- **AST 级优化器（`-opt`，`src/backend/opt/Optimizer.cpp`）**：位于语义检查与代码生成之间。
  ToyC 无指针/数组、别名分析平凡，可安全实现真正的中端数据流优化，各 pass 迭代至不动点：
  **全局常量传播**（全程未被赋值的全局内联为常量，初始化链迭代折叠）、**局部常量/复写传播**
  （作用域化顺序数据流，`if` 分支交集合并、循环按赋值集合失效）、**常量折叠与代数化简**
  （32 位回绕；`x+0`/`x*1`/`x*0`/`x-x` 等；`(x+c1)+c2` 重结合；`0&&e`/`1||e` 短路定值）、
  **公共子表达式消除**（线性区间内纯右值复用）、**死代码删除**（mark-sweep，可删除整个无效
  循环；`while(1)` 不删）、**循环不变量外提**（纯不变子表达式提升为 `@licmN` 临时量并跨出现
  点去重）、**常量分支消除**。配合代码生成层的**操作数直连**（寄存器/`zero`/提升常量直接作为
  指令操作数，`x = y op z` 单指令直达）与**循环内算术常量提升**（乘/除/模常量与超立即数加减
  常量进空闲寄存器）。最后补充：循环中可求值的**全局常量**和大立即数无论出现在比较、实参、
  返回值或算术位置都会提升；浅无调用函数可使用 `a1..a7` 加 `t2..t5` 容纳至多 11 个局部；
  值语境 `&&/||` 直连条件分支，可变全局读写使用 `lui + lw/sw`。
...
- 功能测试：`bash tests/scripts/run_tests.sh [-opt]`。以本机 `gcc` 把 `.tc` 当 C 编译运行得到期望
  退出码，与本编译器产物在 `qemu-riscv32` 上的退出码逐一比对。当前 42 个功能用例在 `-opt` 与
  非 `-opt` 下均全部通过。
- 性能对比：`bash tests/scripts/perf_compare.sh [用例.tc]`（默认 `tests/cases/perf/prime.tc`）。
  ...
  两者退出码均为 192；最终 3 次取最小结果为本项目 `0.22s`、GCC `-O2` `0.19s`，性能得分
  `min(1, 基准/实际)` 为 `0.86`。引入 AST 级优化器后，典型优化模式的本地基准（常量链、
  死代码循环、复写链、公共子表达式、代数化简、循环不变量、综合场景）达到或接近 gcc -O2
  （比值 ~1.0）；尾递归压力用例（`sum(1000000)` × 200 次）无递归 `call`、参数回填零访存，
  比值约 `0.73`。10 项语义对抗用例（短路副作用、含调用的 `x-x`、除零、可变全局、遮蔽复写、
  交换参数尾递归、条件除法 LICM、副作用死赋值、常量分支、INT_MIN 回绕）与 gcc 行为全部一致。

测试脚本已统一为 LF 行尾，可直接在 WSL 的 Bash 环境执行。
