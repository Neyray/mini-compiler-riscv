...
这些优化只在 `-opt` 下启用；通用循环不变量外提、跨基本块复制传播和全局公共子表达式消除仍未引入，以避免没有线性 IR/数据流分析时的语义风险。
...
- 后端：RISC-V32 代码生成（D）
- 后端优化：寄存器分配、常量折叠/传播、短路跳转、强度削弱、窥孔、寄存器直连、`addi` 自更新、单参数直传、小栈帧、**小叶子函数内联**、**循环条件常量提升**、**叶子函数无栈帧（a1..a7）**、**条件落空布局 + 循环旋转**、**安全代数化简/死语句删除**、**自尾递归消除**、常量/寄存器直写（D）
