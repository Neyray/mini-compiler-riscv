...
- **优化（`-opt`）**：常量折叠/传播（含全局常量内联）、乘 2 的幂强度削弱为移位、寄存器变量之间的
  ...
  移位化简（有符号截断除法与算术右移对负数不等价）。此外还包括**叶子函数无栈帧**（局部改放 `a1..a7`）
  与 `if`/`while` 的**条件落空布局 + 循环旋转**两项热路径优化。
...
- 性能对比：`bash tests/scripts/perf_compare.sh [用例.tc]`（默认 `tests/cases/perf/prime.tc`）。
  ...
  两者退出码均为 192；引入「叶子函数无栈帧」与「条件落空布局 + 循环旋转」后，本项目 `-opt` 产物的
  最佳时间（12 次取最小）由 `0.32s` 降至 `0.27s`（相对基线快约 18%），GCC `-O2` `0.18s`，性能得分
  `min(1, 基准/实际)` 由 `0.56` 提升至 `0.67`。时间会随机器和 QEMU 版本波动。
