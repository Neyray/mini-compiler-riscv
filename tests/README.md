...
> 现状：`cases/functional/` 已含 42 个功能用例，`scripts/run_tests.sh [-opt]` 以本机 gcc 为基准
> 自动判分（普通与 `-opt` 模式均 42/42 通过）；`cases/perf/` + `scripts/perf_compare.sh` 用于与
> `gcc -O2` 对比性能（`prime.tc` 上性能比 `min(1, 基准/实际)` 约 `0.86`）；
