...
- **自尾递归消除**：仅匹配函数末尾的 `return 当前函数(实参...)`。所有新实参先写入已有临时槽，再回填形参并跳转到函数体入口，因此交换形参等情形也不会发生覆盖；非尾递归和间接递归继续走普通调用路径。
...
这些优化只在 `-opt` 下启用；通用循环不变量外提、跨基本块复制传播和全局公共子表达式消除仍未引入，以避免没有线性 IR/完整数据流分析时的语义风险。

在 WSL2 + `qemu-riscv32` 上，`tests/cases/perf/prime.tc` 的验证结果为：项目 `-opt` 产物与
GCC `-O2` 产物的退出码均为 `192`；最终复测的 3 次取最小结果分别为 `0.21s` 与 `0.18s`，
性能比 `min(1, gcc-O2/ours)` 为 **`0.86`**，与本轮开始前的稳定记录相同。该素数基准不包含
尾递归或明显冗余表达式，因此本轮新增规则在该单一基准上的可量化提升为 0%。独立的 100,000 次
自尾递归压力用例已验证生成代码无递归 `call` 且返回值正确。
...
- 后端：RISC-V32 代码生成（D）
- 后端优化：寄存器分配、常量折叠/传播、短路跳转、强度削弱、窥孔、寄存器直连、`addi` 自更新、单参数直传、小栈帧、**小叶子函数内联**、**循环条件常量提升**、**叶子函数无栈帧（a1..a7）**、**条件落空布局 + 循环旋转**、**安全代数化简/死语句删除**、**局部前向常量传播**、**相邻纯 CSE**、**块作用域存储复用**、**自尾递归消除**、常量/寄存器直写（D）
