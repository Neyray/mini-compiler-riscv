...
- 性能对比：`bash tests/scripts/perf_compare.sh [用例.tc]`（默认 `tests/cases/perf/prime.tc`）。
  ...
  两者退出码均为 192；依次引入「叶子函数无栈帧」「条件落空布局 + 循环旋转」「小叶子函数内联」
  「循环条件常量提升」后，本项目 `-opt` 产物的最佳时间（12 次取最小）由 `0.32s` 降至 `0.21s`
  （用时约为基线的 66%），GCC `-O2` `0.18s`，性能得分 `min(1, 基准/实际)` 由 `0.56` 提升至
  `0.86`。时间会随机器和 QEMU 版本波动。
