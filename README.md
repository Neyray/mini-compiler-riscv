...
`0.32s` 降至 `0.21s`（用时约为基线的 66%），GCC `-O2` 为 `0.18s`，性能比
`min(1, gcc-O2/ours)` 由 `0.56` 提升至 **`0.86`**。优化后热点函数 `isPrime` 被完全内联进
`main`，热路径上不再有任何 `call` 与冗余 `li`。该时间受机器与 QEMU 版本影响，提交前应在目标环境复测。
...
- 后端：RISC-V32 代码生成（D）
- 后端优化：寄存器分配、常量折叠/传播、短路跳转、强度削弱、窥孔、寄存器直连、`addi` 自更新、单参数直传、小栈帧、**小叶子函数内联**、**循环条件常量提升**、**叶子函数无栈帧（a1..a7）**、**条件落空布局 + 循环旋转**、常量/寄存器直写（D）
